...
from scripts.lib.bench_sdd import toolchain_image


class ToolchainImageTest(unittest.TestCase):
    def test_scenario_without_dockerfile_uses_base(self):
        self.assertEqual(
            toolchain_image("benchmarks/smoke/scenario.yaml"),
            "dail-toolchain")

    def test_scenario_with_dockerfile_uses_overlay(self):
        with tempfile.TemporaryDirectory() as d:
            sd = pathlib.Path(d) / "myscenario"
            sd.mkdir()
            (sd / "Dockerfile").write_text("FROM x")
            self.assertEqual(toolchain_image(str(sd / "scenario.yaml")), "dail-toolchain-myscenario")

    def test_scenario_without_dockerfile_uses_base_synthetic(self):
        with tempfile.TemporaryDirectory() as d:
            sd = pathlib.Path(d) / "plain"
            sd.mkdir()
            self.assertEqual(toolchain_image(str(sd / "scenario.yaml")), "dail-toolchain")
